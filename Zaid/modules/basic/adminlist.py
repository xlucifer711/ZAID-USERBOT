...
@Client.on_message(filters.me & filters.command(["المشرفين", "الادمن"], "."))
async def adminlist(client: Client, message: Message):
    ...
    for x in admin:
        ...
        if len(teks) >= 4096:
            ...
            teks = ""
            toolong = True
    teks += "╞══「 {} البــوتــاتــ الـمــشــࢪفــيــنـ 」\n".format(len(badmin))
    for x in badmin:
        teks += "│ • {}\n".format(x)
        if len(teks) >= 4096:
            await message.reply(message.chat.id, teks, reply_to_message_id=replyid)
            teks = ""
            toolong = True
    teks += "╘══「 عــدد {} الـمــشــࢪفــيــنـ 」".format(totaladmins)
    if toolong:
        await message.reply(message.chat.id, teks, reply_to_message_id=replyid)
    else:
...
@Client.on_message(
    filters.me & filters.command(["ابلاغ", "بلغ الادمن", "بلاغ"], ".")
)
...
@Client.on_message(filters.me & filters.command(["تاج للكل", "mentionall"], "."))
...
@Client.on_message(filters.me & filters.command(["قايمه البوتات", "البوتات"], "."))
async def get_list_bots(client: Client, message: Message):
    ...
    for x in bots:
        teks += "│ • {}\n".format(x)
    teks += "╘══「 عــدد {} البــوتــاتــ 」".format(len(bots))
    if replyid:
...
add_command_help(
    "tag",
    [
        [f".المشرفين", "الادمن ال في البار."],
        [f".صفي الحذفين or .الحذفين", "طرد ال حسبتهم محذوفه."],
        [
            f".تاج للكل `or` .mentionall",
            "تعمل تاج للناس ",
        ],
        [
            f".قايمه البوتات",
